...
function list_accounts() {
    ...
    gh auth status --show-token 2>/dev/null | grep "Logged in to github.com account" | \
        sed -E 's/.*account ([^ ]+).*/\1/' | nl -w2 -s') '
}
...
while true; do
    ...
    if [ -n "$current_user" ]; then
        echo
        echo "=== $APP_NAME ==="
        echo "Akun aktif: $current_user@github.com"
    else
        echo
        echo "=== $APP_NAME ==="
        echo "Akun aktif: (belum login)"
    fi
